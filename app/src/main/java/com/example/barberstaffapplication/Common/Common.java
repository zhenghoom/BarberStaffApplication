package com.example.barberstaffapplication.Common;
...
import com.example.barberstaffapplication.Service.MyFCMService;
...
public class Common {
        ...
        if(fileUri.getScheme().equals("content"))
        {
            ...
            try{
                if(cursor != null && cursor.moveToFirst())
                    result = cursor.getString(cursor.getColumnIndex(OpenableColumns.DISPLAY_NAME)); //Need Supress range
            }finally {
                cursor.close();
            }
        }
    ...
    public static void updateToken(Context context, String token) {
        //Check if user login because need store token of user
        Paper.init(context);
        ...
        if (user != null)
        {
            if(!TextUtils.isEmpty(user))
            {
                ...
                myToken.setTokenType(TOKEN_TYPE.BARBER); //Because run from barber app
